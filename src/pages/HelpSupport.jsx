...
//   const API_URL = "https://quickcabpune.com/app/help_support?page=1&limit=500&search=a";
...
const HelpSupport = () => {
  ...
  const saveRemark = async () => {
    ...
    try {
      ...
      await axios.post(`${BASE_URL}/${selectedId}/remark`, requestData);
      fetchData(currentPage, rowsPerPage);
      closeRemarkForm();
    } catch (error) {
      console.error("Error updating remark:", error);
  ...
  const formatDate = (dateString) => {
    ...
    const date = new Date(dateString);
    return isNaN(date) || date.getFullYear() < 1900 ? "" : date.toLocaleString();
  };
  ...
  return (
    <div className="help_support">
      <div className="d-flex align-items-center justify-content-between flex-wrap">
        ...
        <div className="rows-per-page">
          <select
            className="requiredData-dropdown"
            id="rowsPerPage"
            value={rowsPerPage}
            onChange={handleRowsPerPageChange}
          >
